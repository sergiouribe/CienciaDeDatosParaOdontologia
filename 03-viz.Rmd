...
# '3-4','3-5','3-6','3-7', 3-8), including the null vector c('') to show no
# solutions.
solutions_shown <- ""
...
```{asis lc-all_alaska_flights-solutions, include=show_solutions('4-3')}
**Learning Check Solutions**  
`flights` contains all flight data, while `all_alaska_flights` contains only data from Alaskan carrier "AS". We can see that flights has `r nrow(flights)` rows while `all_alaska_flights` has only `r nrow(all_alaska_flights)`
```
...
```{asis lc-scatterplots-solutions, include=show_solutions('4-3')}
**Learning Check Solutions**  
**`r paste0("(LC", chap, ".", (lc - 3), ")")`**: What are some practical reasons why `dep_delay` and `arr_delay` have a positive relationship? *The later a plane departs, typically the later it will arrive.*  
**`r paste0("(LC", chap, ".", (lc - 2), ")")`** An example in the `weather` dataset is `visibility`, which measure visibility in miles. As visibility increases, we would expect departure delays to decrease. 

**`r paste0("(LC", chap, ".", (lc - 1), ")")`**: What does (0, 0) correspond to from the point of view of a passenger on an Alaskan flight?  Why do you believe there is a cluster of points near (0, 0)? *The point (0,0) means no delay in departure and arrival. From the passenger's point of view, this means the flight was on time. It seems most flights are at least close to being on time.*  

**`r paste0("(LC", chap, ".", (lc), ")")`**: Create a similar plot, but one showing the relationship between departure time and departure delay. What hypotheses do you have about the patterns you see? *We now put `dep_time` as the `x`-aesthetic and `dep_delay` as the `y`-aesthetic*  
```
...
The large mass of points near (0, 0) in Figure \@ref(fig:noalpha) can cause some confusion.  This is the result of a phenomenon called *over-plotting*.  As one may guess, this corresponds to values being plotted on top of each other _over_ and _over_ again.  It is often difficult to know just how many values are plotted in this way when looking at a basic scatterplot as we have here. There are two ways to address this issue:
...
The key feature to note in Figure \@ref(fig:alpha) is that the transparency of the points is cumulative: areas with a high-degree of over-plotting are darker, whereas areas with a lower degree are less dark. 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** After viewing the Figure \@ref(fig:alpha) above, give an approximate range of arrival times and departure times that occur the most frequently.  How has that region changed compared to when you observed the same plot without the `alpha = 0.2` set in Figure \@ref(fig:noalpha)?

```{asis lc-overplotting-solutions, include=show_solutions('4-3')}
**Learning Check Solutions**  
...
```{asis lc-early_january_weather-solutions, include=show_solutions('4-4')}
**Learning Check Solutions**  
...
```{asis lc-linegraph-solutions, include=show_solutions('4-4')}
**Learning Check Solutions**  
...
```{asis lc-histogram-solutions, include=show_solutions('4-5')}
**Learning Check Solutions**  
...
```{asis lc-facet-solutions, include=show_solutions('4-6')}
**Learning Check Solutions**  
...
**`r paste0("(LC", chap, ".", (lc - 1), ")")`: For which types of datasets would these types of faceted plots not work well in comparing relationships between variables? Give an example describing the variability of the variables and other important characteristics?**
...
```{asis lc-boxplot-solutions, include=show_solutions('4-7')}
**Learning Check Solutions**  
...
```{r, echo=show_solutions('4-7'), eval=FALSE}
weather %>% 
  filter(month==5 & temp < 25)
```

```{r, include=show_solutions('4-7'), echo=FALSE}
weather %>% 
  filter(month==5 & temp < 25) %>% 
  kable()
```

```{asis, include=show_solutions('4-7')}
...
```{r, echo=FALSE, include=show_solutions('4-7')}
weather %>% 
  group_by(month) %>% 
  summarize(IQR = IQR(temp, na.rm=TRUE)) %>% 
  arrange(desc(IQR)) %>% 
...
```{asis lc-barplot-solutions, include=show_solutions('4-8')}
**Learning Check Solutions**
...
```{asis lc-pie-charts-solutions, include=show_solutions('4-8')}
**Learning Check Solutions**
...
```{asis lc-barplot-two-var-solutions, include=show_solutions('4-8')}
**Learning Check Solutions**
...
```{asis lc-barplot-stacked-solutions, include=show_solutions('4-9')}
**Learning Check Solutions**
...
Note how the `facet_grid` function arguments are written here.  We are wanting the names of the airports vertically and the `carrier` listed horizontally.  As you may have guessed, this argument and other _formulas_ of this sort in R are in `y ~ x` order.  We will see more examples of this in Chapter \@ref(regression). <!--Nina says look at this.-->

If you'd like to create small multiples in a vertical direction, you'll want to use `facet_grid()` with the name of the variable before the `~` as we did in Figure \@ref(fig:facet-bar-vert).  This corresponds to vertical going with `y` in the formula.  If instead you'd like the small multiples to be in the horizontal direction, you'd use `facet_grid()` with the name of the variable after the `~`, corresponding to the `x` position in the formula.  Further, you can use `facet_wrap()` if you would like the small multiples to wrap into multiple rows as we saw earlier in the faceted histogram example in Figure \@ref(fig:facethistogram).  Additionally, you could use `facet_grid()` with one variable in the `y` position and another variable in the `x` position creating a grid of all possible combinations of the two variables.
...
```{asis lc-barplot-facet-solutions, include=show_solutions('4-8')}
**Learning Check Solutions**  
...
- [Scatterplots & Linegraphs](https://campus.datacamp.com/courses/effective-data-storytelling-using-the-tidyverse/scatter-plots-line-graphs)
...
In Chapter \@ref(tidy), we'll introduce the concept of "tidy data" and how it is used as the driving force behind data visualizations and the remainder of the textbook.  You'll see that the concept appears to be simple, but actually can be a little challenging to decipher without careful practice. We'll also investigate how to import CSV (comma-separated value) files into R using the `readr` package.  
...
knitr::purl("03-viz.Rmd", "docs/scripts/03-viz.R")
```

An R script file of all R code used in this chapter is available [here](http://moderndive.com/scripts/03-viz.R).
