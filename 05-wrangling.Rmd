...
* `sum()`: the sum
* `n()`: a count of the number of rows/observations in each group. This particular summary function will make more sense when `group_by()` is covered in Subsection \@ref(groupby).
...
Run the following code (recall the discussion of the `kable()` function from the `knitr` package in Subsection \@ref(explore-dataframes)):
...
* Then `gain = dep_delay - arr_delay = 10 - 20  = -10` is negative, so it "made
up time in the air".
...
An R script file of all R code used in this chapter is available [here](http://moderndive.com/scripts/05-manip.R).
